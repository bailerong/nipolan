...
public class LexicalAnalyzer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //输出文件
        FileOutputStream fileOutputStream = new FileOutputStream(new File("D://Out.txt"));
        ...
        while (afterfile[index] != '\0') {
            ...
            if (la.isLetter(afterfile[index])) {
                ...
                if (la.isReserve(temp, reserve) != -1) {
                    ...
                    String str = "保留字:(" + la.isReserve(temp, reserve) + "," + temp + ")" + "\r\n";
                    byte[] bytes = str.getBytes();
                    fileOutputStream.write(bytes);
                } else {
                    System.out.println("标识符:(" + 100 + "," + temp + ")");
                    String str = "标识符:(" + 100 + "," + temp + ")" + "\r\n";
                    byte[] bytes = str.getBytes();
                    fileOutputStream.write(bytes);
                }
            }
            ...
            //既不是数字也不是字母也不是空格，则为界符或运算符，跳过空格
                ...
                if (la.isDivide(temp, divide) != -1) {
                    ...
                    String str = "界符:(" + la.isDivide(temp, divide) + "," + temp + ")" + "\r\n";
                    byte[] bytes = str.getBytes();
                    fileOutputStream.write(bytes);
                    temp = "";
                    index++;
                ...
                //判断是否为运算符
                    ...
                    if (la.isOperator(temp, operator) != -1) {
                        ...
                        String str = "运算符:(" + la.isOperator(temp, operator) + "," + temp + ")" + "\r\n";
                        byte[] bytes = str.getBytes();
                        fileOutputStream.write(bytes);
                    } else {
                        System.out.println("无法识别，可能为中文字符");
                        String str = "无法识别，可能为中文字符" + "\r\n";
                        byte[] bytes = str.getBytes();
                        fileOutputStream.write(bytes);
                    }
                }
